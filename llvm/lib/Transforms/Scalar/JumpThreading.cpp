...
// If BB has exactly one predecessor with a conditional branch to BB, the edge
// taken into BB determines the branch condition value (true for successor 0,
// false for successor 1). Materialize that fact by replacing uses in BB.
static bool propagatePredBranchConditionIntoBlock(BasicBlock *BB,
  ...
  auto *CondTy = dyn_cast<IntegerType>(Cond->getType());
  if (!CondTy || CondTy->getBitWidth() != 1)
    return false;

  auto getI1Constant = [](Value *V) -> std::optional<bool> {
  ...
  while (!Worklist.empty()) {
    ...
    if (auto *BO = dyn_cast<BinaryOperator>(I)) {
      ...
      if (BO->getOpcode() == Instruction::Xor) {
        if (std::optional<bool> C = getI1Constant(BO->getOperand(1))) {
          Worklist.emplace_back(BO->getOperand(0), KnownVal ^ *C);
          continue;
        }
        if (std::optional<bool> C = getI1Constant(BO->getOperand(0))) {
    ...
    if (auto *SI = dyn_cast<SelectInst>(I)) {
      ...
      std::optional<bool> TC = getI1Constant(TV);
      std::optional<bool> FC = getI1Constant(FV);
      if (KnownVal) {
        if (TC && !*TC) {
  ...
  SmallVector<std::pair<Use *, Constant *>, 16> UsesToReplace;
  for (const auto &KV : KnownFacts) {
    ...
    for (Use &U : KnownV->uses()) {
      ...
      if (!UserI || UserI->getParent() != BB || isa<PHINode>(UserI))
        continue;
      UsesToReplace.emplace_back(&U, KnownC);
    }
  }
  ...
  for (const auto &P : UsesToReplace)
...
bool JumpThreadingPass::tryThreadEdge(
    ...
    BasicBlock *SuccBB) {
  // If multiple predecessors are provided, try threading them individually
  // (one-by-one). This conservatively creates separate cloned blocks for
  // each predecessor, which preserves path-specific facts and allows LVI to
  // discover constants per-edge. If any single-pred threading succeeds,
  // report success.
  if (PredBBs.size() > 1) {
    bool Any = false;
    for (BasicBlock *P : PredBBs) {
      SmallVector<BasicBlock *, 1> Single;
      Single.push_back(P);
      if (tryThreadEdge(BB, Single, SuccBB))
        Any = true;
    }
    return Any;
  }
...
void JumpThreadingPass::threadEdge(BasicBlock *BB,
  ...
  // Remember original preds for path-sensitive queries.
  SmallVector<BasicBlock *, 8> OrigPreds(PredBBs.begin(), PredBBs.end());
  ...
  // Try to fold live-in values that are known to be constant on the edge
  // (PredBB -> BB) into constants inside the cloned block NewBB. This makes
  // path-sensitive facts explicit in the IR and allows later passes (e.g.
  // InstCombine/ConstantPropagation) to simplify uses (like stores) to
  // constant values. Only do simple, type-matching replacements restricted
  // to uses inside NewBB to avoid changing semantics elsewhere.
  for (ValueToValueMapTy::const_iterator I = ValueMapping.begin(),
       E = ValueMapping.end(); I != E; ++I) {
    const Value *OrigV = I->first;
    Value *MappedV = I->second;
    if (!MappedV)
      continue;

    // We only consider values that are live-ins to BB (i.e. not defined in BB)
    // since values defined in BB have already been cloned and will be
    // simplified via existing mechanisms.
    if (const Instruction *Inst = dyn_cast<Instruction>(OrigV)) {
      if (Inst->getParent() == BB)
        continue;
    }

    // Use the more powerful computeValueKnownInPredecessors to determine
    // whether OrigV has a known constant on each incoming edge (Pred -> BB)
    // for the original predecessors in PredBBs. Only replace when every
    // original predecessor provides the same constant.
    PredValueInfoTy PredVals;
    if (!computeValueKnownInPredecessors(const_cast<Value *>(OrigV), BB,
                       PredVals, WantInteger,
                       BB->getTerminator()))
      continue;

    Constant *C = nullptr;
    bool AllAgree = true;
    for (BasicBlock *OrigPred : PredBBs) {
      // Find the entry for this predecessor.
      Constant *CPred = nullptr;
      for (auto &PV : PredVals)
        if (PV.second == OrigPred) {
          CPred = PV.first;
          break;
        }
      if (!CPred) { AllAgree = false; break; }
      if (!C)
        C = CPred;
      else if (C != CPred) { AllAgree = false; break; }
    }
    if (!AllAgree || !C)
      continue;

    // Only replace when the constant type matches the mapped value type.
    if (C->getType() != MappedV->getType())
      continue;

    // Replace uses of the mapped value that occur only inside NewBB.
    SmallVector<Use *, 8> UsesToReplace;
    for (Use &U : MappedV->uses()) {
      if (Instruction *UserI = dyn_cast<Instruction>(U.getUser())) {
        if (UserI->getParent() == NewBB)
  ...
  // If we didn't find constants via LVI-based methods, do a conservative
  // backward walk from PredBB up to a small depth to see if some ancestor
  // conditional branch directly tests a boolean value that we can use to
  // replace mapped uses inside NewBB. This handles simple control-dependant
  // facts like "br i1 %x, T, F" where along edge the value of %x is known.
  const unsigned MaxBacktrackDepth = 6;
  for (ValueToValueMapTy::const_iterator I = ValueMapping.begin(),
       E = ValueMapping.end(); I != E; ++I) {
    const Value *OrigV = I->first;
    Value *MappedV = I->second;
    if (!MappedV || !OrigV)
      continue;
    // Only consider i1 typed values for this simple heuristic.
    if (!OrigV->getType()->isIntegerTy(1))
      continue;

    // Walk predecessors starting from PredBB and look for a condbr that
    // directly uses OrigV.
    BasicBlock *ScanBB = PredBB;
    unsigned Depth = 0;
    ConstantInt *FoundC = nullptr;
    while (ScanBB && Depth++ < MaxBacktrackDepth) {
      // Look at single predecessor chain; if multiple preds, stop.
      BasicBlock *Prev = ScanBB->getSinglePredecessor();
      if (!Prev) break;
      if (auto *CB = dyn_cast<CondBrInst>(Prev->getTerminator())) {
        Value *Cond = CB->getCondition();
        if (Cond == OrigV) {
          // Determine which successor corresponds to ScanBB.
          unsigned SuccIdx = (CB->getSuccessor(0) == ScanBB) ? 0 : 1;
          // If successor 0 corresponds, condition==true on that edge.
          FoundC = ConstantInt::get(IntegerType::get(OrigV->getContext(), 1),
                                    SuccIdx == 0);
          break;
        }
      }
    ...
    // Replace uses of MappedV only inside NewBB with FoundC.
    SmallVector<Use *, 8> UsesToReplace2;
    for (Use &U : MappedV->uses()) {
      if (Instruction *UserI = dyn_cast<Instruction>(U.getUser())) {
        if (UserI->getParent() == NewBB)
    ...
    if (UsesToReplace2.empty())
      continue;
    for (Use *U : UsesToReplace2)
      U->set(FoundC);
  }
